...
export default function MaterielPage() {
  ...
  function openIAForEquipement(eq: Equipement) {
    const msg = `Donne-moi un rapport complet sur cet équipement : ${eq.designation} (Réf: ${eq.reference || '—'} | N°Série: ${eq.numero_serie || '—'}) — Localisation: ${eq.localisation || '—'} — Statut: ${eq.statut}`
    const encoded = encodeURIComponent(msg)
    window.location.href = `/dashboard/assistant?q=${encoded}`
  }
        ...
        <div onMouseDown={e => { if (e.target === e.currentTarget) { setSelected(null); setPannDesc(''); setPanneSuccess(false) } }}
          ...
          <div style={{ background: 'var(--surface)', borderRadius: 'var(--radius-xl)', width: '100%', maxWidth: '560px', maxHeight: '88vh', overflow: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.12)', border: '1px solid var(--border)' }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', position: 'sticky', top: 0, background: 'var(--surface)', zIndex: 10 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0, flexWrap: 'wrap' }}>
                ...
                <button onClick={() => openIAForEquipement(selected)}
                  ...
                  <i className="ti ti-sparkles" style={{ fontSize: '14px' }} aria-hidden="true" />IA
                </button>
                {/* Bouton Modifier */}
            ...
            <div style={{ padding: '20px 24px' }}>
              ...
              {/* Bloc 5 — Historique déplacements */}
              <div style={{ marginBottom: '20px' }}>
                <div style={{ fontSize: '11px', fontWeight: '500', color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.6px', marginBottom: '10px', paddingBottom: '8px', borderBottom: '1px solid var(--border)' }}>Bloc 5 — Historique des déplacements</div>
                {historique.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '12px', color: 'var(--text-tertiary)', fontSize: '12px', fontStyle: 'italic' }}>Aucun déplacement enregistré</div>
                ) : historique.map((h, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '8px 12px', background: 'var(--surface-hover)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)', marginBottom: '6px' }}>
                    ...
                    <i className="ti ti-arrow-right" style={{ fontSize: '13px', color: 'var(--text-tertiary)', flexShrink: 0 }} aria-hidden="true" />
                    <span style={{ fontSize: '12px', fontWeight: '500', color: 'var(--text-primary)', flex: 1 }}>{h.nouvelle_localisation || '—'}</span>
                    <span style={{ fontSize: '11px', color: 'var(--text-tertiary)', flexShrink: 0 }}>{new Date(h.created_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                  </div>
                ))}
              </div>
          ...
          <div style={{ background: 'var(--surface)', borderRadius: 'var(--radius-xl)', width: '100%', maxWidth: '600px', maxHeight: '90vh', overflow: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.12)', border: '1px solid var(--border)' }}>
            ...
            <div style={{ padding: '20px 24px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                ...
                <div><label style={labelStyle}>Prochaine révision</label><input type='date' value={editForm.date_revision || ''} onChange={e => setEditForm(p => ({ ...p, date_revision: e.target.value }))} style={inputStyle} /></div>
...
us-md)', color: '#fff', fontSize: '13px', fontWeight: '500', cursor: editSaving || !editForm.designation ? 'not-allowed' : 'pointer', fontFamily: 'var(--font)', boxShadow: '0 1px 4px rgba(26,86,219,0.3)' }}>
          ...
          <div style={{ background: 'var(--surface)', borderRadius: 'var(--radius-xl)', width: '100%', maxWidth: '600px', maxHeight: '90vh', overflow: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.12)', border: '1px solid var(--border)' }}>
            ...
            <div style={{ padding: '20px 24px' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  <label style={{ ...labelStyle, color: !addForm.etablissement_id ? 'var(--warning)' : 'var(--text-secondary)' }}>Établissement — requis pour que le client voit le matériel</label>
                  ...
                  {!addForm.etablissement_id && (
                    <div style={{ fontSize: '11px', color: 'var(--warning)', marginTop: '4px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                      <i className="ti ti-alert-triangle" style={{ fontSize: '12px' }} aria-hidden="true" />Sans établissement, l'équipement n'apparaîtra pas chez le client
                    </div>
                  )}
                </div>
                {[
                ...
                <div><label style={labelStyle}>Prochaine révision</label><input type='date' value={addForm.date_revision} onChange={e => setAddForm(p => ({ ...p, date_revision: e.target.value }))} style={inputStyle} /></div>
                <div style={{ gridColumn: '1 / -1', marginTop: '8px' }}>
                  <div style={{ fontSize: '11px', fontWeight: '600', color: 'var(--accent)', textTransform: 'uppercase', letterSpacing: '0.6px', paddingBottom: '8px', borderBottom: '1px solid var(--border)', marginBottom: '4px' }}>Bloc 6 — Commentaires</div>
                </div>
                <div style={{ gridColumn: '1 / -1' }}>
              ...
              {addError && (
                <div style={{ padding: '10px 14px', background: 'var(--danger-light)', border: '1px solid rgba(194,54,42,0.2)', borderRadius: 'var(--radius-sm)', fontSize: '12px', color: 'var(--danger)', marginTop: '12px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <i className="ti ti-alert-circle" style={{ fontSize: '14px', flexShrink: 0 }} aria-hidden="true" />{addError}
                </div>
              )}
