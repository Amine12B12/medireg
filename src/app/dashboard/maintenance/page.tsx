            ...
            return (
              <div key={m.id} style={{ background: '#fff', borderRadius: '14px', border: '0.5px solid #DDE5F0', padding: '20px 24px', display: 'flex', alignItems: 'center', gap: '20px' }}>
                <div style={{ minWidth: '64px', textAlign: 'center', padding: '10px', background: '#F8FAFC', borderRadius: '10px' }}>
                  {m.date_prevue ? (
                    <>
                      <div style={{ fontSize: '22px', fontWeight: '700', color: '#0A1628', lineHeight: 1 }}>{new Date(m.date_prevue).getDate()}</div>
                      <div style={{ fontSize: '11px', color: '#6B7A99', textTransform: 'uppercase', marginTop: '2px' }}>{new Date(m.date_prevue).toLocaleDateString('fr-FR', { month: 'short' })}</div>
                    </>
                  ) : <div style={{ fontSize: '12px', color: '#DC2626', fontWeight: '600' }}>Urgent</div>}
                </div>
                <div style={{ width: '1px', height: '48px', background: '#F0F4FA' }} />
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '14px', fontWeight: '600', color: '#0A1628' }}>{m.equipements?.designation} — {m.equipements?.reference}</div>
                  <div style={{ fontSize: '12px', color: '#6B7A99', marginTop: '3px' }}>{m.equipements?.localisation} · {typeLabel(m.type)}</div>
                  {m.notes && <div style={{ fontSize: '12px', color: '#6B7A99', marginTop: '4px', fontStyle: 'italic' }}>{m.notes}</div>}
                </div>
            ...
            <div style={{ padding: '24px 28px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                ...
                <button onClick={handleAdd} disabled={saving || !form.equipement_id}
                  style={{ flex: 1, padding: '11px', background: saving || !form.equipement_id ? '#93AEED' : '#1A56DB', border: 'none', borderRadius: '10px', color: '#fff', fontSize: '13px', fontWeight: '500', cursor: 'pointer', fontFamily: 'inherit' }}>
