...
const CRITERE_CONFIG: Record<string, {
  ...
  '2.4.3': {
    ...
    tables: [
      ...
      { table: 'pannes', label: 'Pannes traitées', icon: 'ti-alert-triangle', color: '#D97706', bg: '#FFFBEB' },
    ]
  },
  '2.5.1': {
...
export default function MeditrackWidget({ meditrackEtabId, critereCode, userEmail, onLink }: Props) {
  ...
  useEffect(() => {
    if (meditrackEtabId) loadStats(meditrackEtabId)
  }, [meditrackEtabId])
  ...
  async function loadStats(etabId: string) {
    ...
    for (const t of config.tables) {
      ...
      if (t.statut_field && t.statut_value) {
        query = query.eq(t.statut_field, t.statut_value)
      }
    ...
    <div style={{ background: '#F8FAFF', border: '1px solid #BFDBFE', borderRadius: '12px', padding: '20px', marginTop: '16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', flexWrap: 'wrap', gap: '10px' }}>
        ...
        <a href="https://www.meditrack-app.fr" target="_blank"
          style={{ padding: '7px 14px', background: '#1A56DB', border: 'none', borderRadius: '8px', color: '#fff', fontSize: '12px', fontWeight: '600', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '5px' }}>
